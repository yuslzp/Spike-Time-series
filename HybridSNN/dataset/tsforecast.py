...
class DATASETS(metaclass=Registry, name="dataset"):
    """Registry for dataset builders used by experiment configs."""

    pass


class TimeFeature:
    """Base class for time-derived scalar features."""

    def __init__(self):
        """Initialize the stateless time feature."""
        pass

    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Transform a datetime index into a normalized numeric feature."""
        pass
...
class SecondOfMinute(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode seconds within the current minute."""
...
class MinuteOfHour(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode minutes within the current hour."""
...
class HourOfDay(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode hours within the current day."""
...
class DayOfWeek(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode weekdays within the current week."""
...
class DayOfMonth(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode days within the current month."""
...
class DayOfYear(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode days within the current year."""
...
class MonthOfYear(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode months within the current year."""
...
class WeekOfYear(TimeFeature):
    ...
    def __call__(self, index: pd.DatetimeIndex) -> np.ndarray:
        """Encode ISO week numbers within the current year."""
...
class TSMSDataset(Dataset):
    """Windowed multivariate time-series dataset for forecasting experiments."""
    ...
    def __init__(
        ...
        """Load, normalize, and split a multivariate forecasting dataset."""
        self.window = window
    ...
    def _normalized(self, normalize):
        """Apply the requested normalization strategy to the raw data."""
    ...
    def _split(self, train_ratio, test_ratio, dataset_name):
        """Compute the active split length and its start offset."""
    ...
    def freeup(self):
        """Release dataset-side cached tensors when no cleanup is needed."""
        pass

    def load(self):
        """Load dataset data eagerly when no additional work is required."""
        pass

    def get_forecast_residual_meta(self) -> dict:
        """Return the target-space transform for last-value residual forecasts."""
        signal_size = self.raw_data.shape[1]
        signal_indices = (
            np.array([signal_size - 1], dtype=np.int64)
            if self.last_label
            else np.arange(signal_size, dtype=np.int64)
        )
        center = np.zeros(signal_size, dtype=np.float32)
        scale = np.ones(signal_size, dtype=np.float32)
        eps = np.finfo(np.float32).eps

        if self.normalize == 1:
            scale.fill(float(np.max(self.raw_data)))
        elif self.normalize == 2:
            scale = np.max(np.abs(self.raw_data), axis=0).astype(np.float32)
        elif self.normalize == 3:
            center.fill(float(np.mean(self.raw_data)))
            scale.fill(float(np.std(self.raw_data) + eps))
        elif self.normalize == 4:
            center = self._col_mean.reshape(-1).astype(np.float32)
            scale = self._col_std.reshape(-1).astype(np.float32)
        elif self.normalize == 5:
            center = self._col_median.reshape(-1).astype(np.float32)
            scale = self._col_iqr.reshape(-1).astype(np.float32)

        scale = np.where(np.abs(scale) < eps, 1.0, scale).astype(np.float32)
        return {
            "raw_label": bool(self.raw_label),
            "signal_indices": signal_indices,
            "target_center": center[signal_indices].astype(np.float32),
            "target_scale": scale[signal_indices].astype(np.float32),
        }

    @property
    def num_variables(self):
        """Return the model input width, including appended time features."""
    ...
    def __len__(self):
        """Return the number of windows available in the active split."""
        return self.length

    @property
    def max_seq_len(self):
        """Return the input window length consumed by the model."""
        return self.window

    @property
    def num_classes(self):
        """Return the flattened forecasting target size for the current setup."""
        if self.last_label:
            return self.horizon
        else:
    ...
    def get_index(self):
        """Return the local sample indices for prediction exports."""
        return np.arange(self.length)

    def __getitem__(self, index):
        """Return one input window and its forecast target vector."""
