...
class SHDDataset(Dataset):
    ...
    def __init__(
        ...
        test_file: str = None,
        time_jitter_bins: int = 0,
        time_scale_range: float = 0.0,
        time_mask_ratio: float = 0.0,
        neuron_dropout: float = 0.0,
        spike_noise_rate: float = 0.0,
    ):
        """Initialize split selection metadata and training augmentations."""
        self.file = file
        ...
        self.time_jitter_bins = max(0, int(time_jitter_bins))
        self.time_scale_range = float(max(0.0, time_scale_range))
        self.time_mask_ratio = float(max(0.0, time_mask_ratio))
        self.neuron_dropout = float(max(0.0, neuron_dropout))
        self.spike_noise_rate = float(max(0.0, spike_noise_rate))
        # When test_file is set, use it as the dedicated test split
    ...
    def get_index(self):
        """Return the sample indices assigned to the active split."""
    ...
    def _apply_time_scaling(self, x: torch.Tensor) -> torch.Tensor:
        """Apply a global speed perturbation while preserving tensor shape."""
        if self.time_scale_range <= 0:
            return x

        scale = float(
            np.random.uniform(1.0 - self.time_scale_range, 1.0 + self.time_scale_range)
        )
        if abs(scale - 1.0) < 1e-6:
            return x

        center = 0.5 * (self.num_time_bins - 1)
        time_idx = torch.arange(self.num_time_bins, dtype=torch.float32)
        src_idx = torch.round((time_idx - center) / scale + center).long()
        src_idx = src_idx.clamp_(0, self.num_time_bins - 1)
        return x.index_select(dim=0, index=src_idx)
    ...
    def __getitem__(self, index: int):
        ...
        x = torch.from_numpy(self._data[index].copy())       # (L, C) float32
        if self.dataset_name == "train":
            if self.time_scale_range > 0:
                x = self._apply_time_scaling(x)
            if self.time_jitter_bins > 0:
                shift = int(np.random.randint(-self.time_jitter_bins, self.time_jitter_bins + 1))
                if shift != 0:
                    x = torch.roll(x, shifts=shift, dims=0)
            if self.time_mask_ratio > 0:
                time_mask = torch.rand(self.num_time_bins) < self.time_mask_ratio
                if time_mask.any():
                    x[time_mask] = 0.0
            if self.neuron_dropout > 0:
                neuron_mask = torch.rand(self.num_neurons) < self.neuron_dropout
                if neuron_mask.any():
                    x[:, neuron_mask] = 0.0
            if self.spike_noise_rate > 0:
                noise = (torch.rand_like(x) < self.spike_noise_rate).float()
                if self.spike_count:
                    x = x + noise
                else:
                    x = torch.maximum(x, noise)
